...
public class UpdateGoogleSheetsTaskTests
{
    ...
    {
        _dataTransferServiceMock.Setup(x => x.GetFieldAsync(DataTransferKeys.ReleaseDataHash, DataTransferKeys.Fields.Consolidated))
            .ReturnsAsync(result?.ToJson());
    }

    private void SetupRedisEnhancedTitlesData(ConsolidatedReleaseResult? result)
    {
        _dataTransferServiceMock.Setup(x => x.GetFieldAsync(DataTransferKeys.ReleaseDataHash, DataTransferKeys.Fields.EnhancedTitles))
            .ReturnsAsync(result?.ToJson());
    }
    ...
    public async Task ExecuteAsync_ShouldUseEnhancedTitles_WhenAvailable()
    {
        ...
        _dataTransferServiceMock.Verify(
            x => x.GetFieldAsync(DataTransferKeys.ReleaseDataHash, DataTransferKeys.Fields.EnhancedTitles),
            Times.Once);
        // Assert: Consolidated 不被讀取
        _dataTransferServiceMock.Verify(
            x => x.GetFieldAsync(DataTransferKeys.ReleaseDataHash, DataTransferKeys.Fields.Consolidated),
            Times.Never);
    }
    ...
    public async Task ExecuteAsync_EndToEnd_ShouldFollowCorrectOrder()
    {
        ...
        _dataTransferServiceMock.Setup(x => x.GetFieldAsync(It.IsAny<string>(), It.IsAny<string>()))
