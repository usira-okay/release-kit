...
public class GetReleaseSettingTaskTests
{
    ...
    public GetReleaseSettingTaskTests()
    {
        ...
        // 預設 Redis 行為
        _dataTransferServiceMock.Setup(x => x.ExistsValueAsync(It.IsAny<string>())).ReturnsAsync(false);
        _dataTransferServiceMock
            .Setup(x => x.SetValueAsync(DataTransferKeys.ReleaseSetting, It.IsAny<string>(), It.IsAny<TimeSpan?>()))
    ...
    {
        // Arrange
        _dataTransferServiceMock.Setup(x => x.GetFieldAsync(DataTransferKeys.GitLabHash, DataTransferKeys.Fields.ReleaseBranches))
            .ReturnsAsync((string?)null);
        _dataTransferServiceMock.Setup(x => x.GetFieldAsync(DataTransferKeys.BitbucketHash, DataTransferKeys.Fields.ReleaseBranches))
            .ReturnsAsync((string?)null);
        ...
        _dataTransferServiceMock.Verify(
            x => x.SetValueAsync(DataTransferKeys.ReleaseSetting, It.IsAny<string>(), It.IsAny<TimeSpan?>()),
            Times.Once);
    }
