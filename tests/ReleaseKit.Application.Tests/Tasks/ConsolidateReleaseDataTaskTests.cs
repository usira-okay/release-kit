...
public class ConsolidateReleaseDataTaskTests
{
    ...
    // ===== T022: 當 Bitbucket 與 GitLab ByUser PR 資料 Key 均不存在時記錄 Warning 並正常結束 =====

    /// <summary>
    /// T022: 測試當 Bitbucket 與 GitLab ByUser PR 資料 Key 均不存在時，記錄 Warning 並正常結束，不拋出例外
    /// </summary>
    [Fact]
    public async Task ExecuteAsync_WithNoPrDataKeys_ShouldLogWarningAndReturn()
    {
        ...
        // Assert — 不寫入 Redis（因為提前結束）
        _redisServiceMock.Verify(
            x => x.HashSetAsync(RedisKeys.ReleaseDataHash, RedisKeys.Fields.Consolidated, It.IsAny<string>()),
            Times.Never);
    }

    // ===== T023: 當 Bitbucket 與 GitLab ByUser PR 資料均為空集合時記錄 Warning 並正常結束 =====
    ...
    // ===== T025: 當 UserStories Work Item 資料 Key 不存在時記錄 Warning 並正常結束 =====

    /// <summary>
    /// T025: 測試當 UserStories Work Item 資料 Key 不存在時，記錄 Warning 並正常結束，不拋出例外
    /// </summary>
    [Fact]
    ...
    // ===== T026: 當 UserStories Work Item 資料為空集合時記錄 Warning 並正常結束 =====
