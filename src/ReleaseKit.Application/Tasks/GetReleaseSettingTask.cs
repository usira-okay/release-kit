...
/// </summary>
/// <remarks>
/// 從資料傳遞存放區讀取前置指令產生的 release branch 資訊，
/// 依規則產生 GitLab 與 Bitbucket 的專案設定，並寫入資料傳遞存放區。
/// </remarks>
public class GetReleaseSettingTask : ITask
{
    ...
    public async Task ExecuteAsync()
    {
        ...
        // 清除舊資料並寫入資料傳遞存放區
        if (await _dataTransferService.ExistsAsync(DataTransferKeys.ReleaseSetting))
        {
            var deleted = await _dataTransferService.DeleteAsync(DataTransferKeys.ReleaseSetting);
            if (deleted)
            {
                _logger.LogInformation("已清除 資料傳遞存放區中的舊 Release Setting 資料");
            }
            else
            {
                _logger.LogWarning("資料傳遞存放區中的舊 Release Setting 資料刪除失敗，Key: {Key}", DataTransferKeys.ReleaseSetting);
            }
        }

        var isWritten = await _dataTransferService.SetAsync(DataTransferKeys.ReleaseSetting, json);
        if (!isWritten)
        {
            _logger.LogWarning("Release Setting 寫入資料傳遞存放區 失敗，Key: {Key}", DataTransferKeys.ReleaseSetting);
            throw new InvalidOperationException($"寫入資料傳遞存放區 失敗，Key: {DataTransferKeys.ReleaseSetting}");
        }

        _logger.LogInformation("Release Setting 已寫入 資料傳遞存放區，Key: {Key}", DataTransferKeys.ReleaseSetting);
