...
public class GetReleaseSettingTask : ITask
{
    ...
    public async Task ExecuteAsync()
    {
        ...
        // 清除舊資料並寫入 Redis
        if (await _dataTransferService.ExistsAsync(RedisKeys.ReleaseSetting))
        {
            var deleted = await _dataTransferService.DeleteAsync(RedisKeys.ReleaseSetting);
            if (deleted)
            {
        ...
        var isWritten = await _dataTransferService.SetAsync(RedisKeys.ReleaseSetting, json);
        if (!isWritten)
        {
    ...
    {
        var json = await _dataTransferService.HashGetAsync(hashKey, field);
